...
- Name: auth\signup.spec.ts >> Effortless AI Home Page >> Verify Login button opens Login page
- Location: tests\auth\signup.spec.ts:17:9
...
Expected: "https://i.goeffortless.ai/"
Received: "https://goeffortless.ai/"
Timeout:  10000ms
...
- img "Hansa CEquity logo — Effortless customer"
...
- img "Partner logo 9"
- img "Partner logo 10"
- paragraph: Effortless Procurement to Pay IDP (Intelligent data processor) is not one more OCR tool; it can do everything a well-trained accountant does to help the accountants to avoid 90% data entry. Effortless fixes the Leakages in expense chaos. It brings the procurement, finance and management teams in one place and gives full visibility, approval and cost control.
- paragraph: Ritesh Menon
- paragraph: Sr. Vice President Finance - Hansa CEquity Group
- list:
  - listitem:
    - button "1"
  - listitem:
...
```ts
  ...
  7  | 
  8  |     async verifyLoginPageLoaded() {
  9  | 
  10 |         await this.page.waitForLoadState('networkidle');
  11 | 
> 12 |         await expect(this.page).toHaveURL(ENV.LOGIN_URL);
     |                                 ^ Error: expect(page).toHaveURL(expected) failed
  13 | 
  14 |         await expect(
  15 |             this.page.getByRole('button', { name: 'Sign Up' })
  16 |         ).toBeVisible();
  17 | 
  18 |     }
  19 | 
